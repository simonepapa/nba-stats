const Player = ({ stats, index }: any) => {
  return (
    <tr>
      <td className="px-5 py-1 border-r text-left">
        {stats.player.first_name.substring(0, 1) + "."} {stats.player.last_name}{" "}
        <span className="opacity-60 text-xs">{stats.player.position}</span>
      </td>
      <td className="px-5 py-1 text-right">{stats.min}</td>
      <td className="px-5 py-1 text-right">{stats.fgm}/{stats.fga}</td>
      <td className="px-5 py-1 text-right">{stats.fg3m}/{stats.fg3a}</td>
      <td className="px-5 py-1 text-right">{stats.ftm}/{stats.fta}</td>
